  ...
  if (integration.id === 'email') {
    return (
      ...
      >
        ...
        <Field label="Usuario / Email"><Input value={imapUser} onChange={e => setImapUser(e.target.value)} placeholder="operations@fixatrip.com" /></Field>
        ...
        <Field label="Usuario SMTP"><Input value={smtpUser} onChange={e => setSmtpUser(e.target.value)} placeholder="operations@fixatrip.com" /></Field>
    ...
    return (
      <Modal integration={integration} isConnected={isConnected} onClose={onClose}
        ...
        footerRight={
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn variant="ghost" onClick={onClose}>Cancelar</Btn>
            <Btn onClick={() => save({ ics_url: fhApiKey || 'https://fareharbor.com/integrations/ics/fixatrippuertorico/calendar/?token=2ae82eb0-39bd-4494-a041-8e9d1759003c' })} disabled={saving}>{saving ? 'Guardando...' : isConnected ? 'Actualizar' : 'Conectar'}</Btn>
          </div>
        }
      >
        ...
        <Field label="URL del Calendario ICS">
          <Input value={fhApiKey || 'https://fareharbor.com/integrations/ics/fixatrippuertorico/calendar/?token=2ae82eb0-39bd-4494-a041-8e9d1759003c'} onChange={e => setFhApiKey(e.target.value)} placeholder="https://fareharbor.com/integrations/ics/..." />
        </Field>
        <div style={{ height: 8 }} />
      </Modal>
    );
