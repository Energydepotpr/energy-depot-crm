    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', background: 'var(--surface)', maxWidth: 480 }}>
      ...
      <div style={{ background: '#1877f2', padding: '16px 20px', color: '#fff' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>Fix a Trip PR</div>
            <div style={{ fontSize: 11, opacity: 0.8 }}>fixatrip.com</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 11, opacity: 0.7 }}>N° Factura</div>
            <div style={{ fontWeight: 700 }}>{invoice_number || 'FAT-001'}</div>
          </div>
        </div>
      </div>
...
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08)">
  <div style="background:#1877f2;padding:20px 24px;color:#fff">
    <div style="font-size:18px;font-weight:700">Fix a Trip PR</div>
    <div style="font-size:12px;opacity:0.8">fixatrip.com</div>
    <div style="margin-top:8px;font-size:13px;opacity:0.9">Factura N° ${invoice.invoice_number || 'FAT-001'}</div>
  </div>
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: 'bot',
      text: '¡Hola! Soy tu asistente de facturación 📄\n\nDime la información de la factura en lenguaje natural. Por ejemplo:\n\n"Factura para María García, tour de snorkeling el 20 de marzo, 3 adultos a $150 cada uno y 1 niño a $80, total $530, link de pago: https://paypal.me/fixatrip/530"',
    }
  ]);
